  ...
  <div>
    <el-container>
      <el-aside width="160px">
        <div class="friendListDiv" v-for="(admin,index) in admins" :key="admin.id" @click="toggleFriend(admin)"
             v-bind:class="{currentChatFriend:currentFriend.id==admin.id}">
          <img :src="admin.userface" class="userfaceImg"/>
          <el-badge :is-dot="isDotMap.get('isDot#'+currentUser.username+'#'+admin.username)!=null">{{admin.name}}</el-badge>
        </div>
...
<script>
  export default{
    ...
    methods: {
      ...
      toggleFriend(admin){
        //切换数据
        if (admin == this.currentFriend) {
          return;
        }
        this.currentFriend = admin;
        this.$store.commit('updateCurrentFriend', admin);
        this.updateChatDiv();
        this.$store.commit("removeValueDotMap", "isDot#" + this.currentUser.username + "#" + admin.username);
        document.getElementById('chatDiv').scrollTop = document.getElementById('chatDiv').scrollHeight;
      },
      loadAdmins(){
        var _this = this;
        this.getRequest("/chat/admins").then(resp=> {
